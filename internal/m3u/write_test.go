...
func TestWriteRoundTrip(t *testing.T) {
	...
	snap := &model.Snapshot{Channels: []model.Channel{
		{ID: "NFL 06", Number: 8506, Name: "NFL 06", Kind: model.KindPlaceholder, LeagueKey: "nfl", StreamURL: "http://x/2"},
	...
	if err := Write(&buf, snap, WriteOptions{GuideTags: true, Now: kick.Add(-time.Hour)}); err != nil {
		t.Fatal(err)
	}
	out := buf.String()
	...
	for _, must := range []string{
		`channel-id="NFL 04"`, `tvg-id="NFL 04"`, `channel-number="8504"`, `tvg-logo="http://logo/nfl.png"`, `group-title="NFL"`,

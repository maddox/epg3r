...
// Every shipped league grounds its art in its own brand color rather than falling back to
...
func TestEveryLeagueDescribesItsArt(t *testing.T) {
	...
	for _, lg := range c.Leagues {
		if lg.Color == DefaultColor {
			t.Errorf("league %s has no color of its own", lg.Key)
		}
		if _, _, _, ok := lg.RGB(); !ok {
			t.Errorf("league %s: color %q does not parse", lg.Key, lg.Color)
		}
		if lg.LogoPath == "" {
